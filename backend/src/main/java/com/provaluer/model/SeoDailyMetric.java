...
@Entity
@Table(name = "seo_daily_metrics", uniqueConstraints = {
    @UniqueConstraint(columnNames = {"page_id", "source", "date"})
})
public class SeoDailyMetric {
    @Id
    ...
    private String source; // 'GSC', 'BING', 'COMPOSITE'
    ...
    public SeoDailyMetric() {}

    public SeoDailyMetric(SeoPage page, String source, LocalDate date, Integer impressions, Integer clicks, BigDecimal ctr, BigDecimal avgPosition) {
        this.page = page;
        this.source = source;
        this.date = date;
